...
export const Reply = <T extends TSchema>(dataType: T) =>
  Type.Union([SuccessReply(dataType), ErrorReply]);

export const RecipeReply = Reply(Recipe);
export type RecipeReply = Static<typeof RecipeReply>;

export const Page = <T extends TSchema>(dataType: T) =>
  Type.Object({
    items: Type.Array(dataType),
